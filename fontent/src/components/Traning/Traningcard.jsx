...
const Traningcard= ({tra}) => {
    const navigate=useNavigate()
    const {id,name,title,image,start,price,certification}=tra
    return (
        <div className='border p-2 rounded-md'>
         <div className='flex-row lg:flex gap-3 items-center'>
         <img className='w-full lg:w-[300px]  rounded-md' src={image} alt="" />
            <div>
                 <h1 className='text-xl font-semibold'>{name}</h1>
                 <h2 className='text-xs text-[#9f9e9e] mb-1'>{title}</h2>
            ...
            <button onClick={()=>navigate(`/singledetails/${id}`)} className='bg-[#2c9acd] transition-all hover:bg-[#377a99f1] w-fit px-3 py-2 text-white font-bold my-2 rounded-md'>View Details</button>
            </div>
         </div>
        </div>
    );
